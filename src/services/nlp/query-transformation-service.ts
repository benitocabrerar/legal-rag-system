...
export class QueryTransformationService {
  ...
  private async extractEntities(query: string): Promise<Entity[]> {
    try {
      ...
      // Convert entity object to Entity array
      const enhancedEntities: Entity[] = [];

      // Process laws
      for (const law of processorEntities.laws || []) {
        const dictEntity = await this.entityDictionary.findEntity(law);
        enhancedEntities.push({
          id: `law_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
          type: EntityType.LAW,
          text: law,
          normalizedText: dictEntity?.normalizedName || law,
          confidence: 0.9,
          startIndex: 0,
          endIndex: 0,
          metadata: dictEntity?.metadata,
          source: dictEntity ? 'hybrid' : 'pattern'
        });
      }

      // Process articles
      for (const article of processorEntities.articles || []) {
        enhancedEntities.push({
      ...
      for (const keyword of processorEntities.keywords || []) {
        const dictEntity = await this.entityDictionary.findEntity(keyword);
        enhancedEntities.push({
          id: `topic_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
          type: EntityType.LEGAL_TOPIC,
          text: keyword,
          normalizedText: dictEntity?.normalizedName || keyword,
          confidence: 0.7,
          startIndex: 0,
          endIndex: 0,
          metadata: dictEntity?.metadata,
  ...
  private async classifyIntent(query: string): Promise<Intent> {
    try {
      const queryIntent = await this.queryProcessor.classifyIntent(query);

      // Convert QueryIntent to Intent type
      return {
        primary: QueryIntent.GENERAL_SEARCH,
  ...
  private buildDebugInfo(
    ...
    const intentClassification = {
      scores: { [intent.primary]: intent.confidence } as Record<QueryIntent, number>,
      reasoning: intent.reasoning || 'No reasoning provided'
    };

    // Create properly typed performance breakdown
    ...
    for (const metric of this.metrics) {
      if (metric.operation in performanceBreakdown) {
        performanceBreakdown[metric.operation as keyof typeof performanceBreakdown] = metric.duration;
      }
    }
